...
export function AIAnalysis({ keywords, visible, onClose }: AIAnalysisProps) {
  ...
  const createPreset = async () => {
    ...
    try {
      const response = await fetch('/api/ai-presets', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newPresetName, prompt: newPresetPrompt }),
      })
  ...
  const deletePreset = async (id: string) => {
    try {
      const response = await fetch(`/api/ai-presets?id=${id}`, { method: 'DELETE' })
      if (response.ok) await loadPresets()
    } catch (error) {
  ...
  return (
    <Portal>
      ...
      >
        ...
        >
          {/* Header */}
          <div className="flex-shrink-0 flex items-center justify-between p-6 border-b" style={{ borderColor: 'var(--glass-border)' }}>
            ...
            <button
              ...
              style={{ color: 'var(--glass-text-secondary)', border: '1px solid var(--glass-border)' }}
            >
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="mb-6 p-3 rounded-lg" style={{ background: 'var(--glass-bg-tertiary)' }}>
              <p className="text-sm" style={{ color: 'var(--glass-text-secondary)' }}>
                <span style={{ color: 'var(--glass-text-primary)', fontWeight: 500 }}>
                  Анализируемые слова ({keywords.length}):
                </span>{' '}
            ...
            <div className="mb-6">
              ...
              {showCreatePreset && (
                <div className="mb-4 p-4 rounded-lg border" style={{ background: 'var(--glass-bg-tertiary)', borderColor: 'var(--glass-border)' }}>
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setShowCreatePreset(false)} className="glass-button px-4 py-2">
                        Отмена
                      </button>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {presets.map((preset) => (
                  <div 
                    ...
                    style={{ background: 'var(--glass-bg-tertiary)', borderColor: 'var(--glass-border)' }}
                  >
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      ...
                      <button
                        ...
                        style={{ color: 'var(--glass-red)', border: '1px solid var(--glass-border)' }}
                      >
              ...
              {presets.length === 0 && !showCreatePreset && (
                <div className="text-center py-8 rounded-lg border" style={{ background: 'var(--glass-bg-tertiary)', borderColor: 'var(--glass-border)' }}>
            ...
            {(analysisResult.analysis || analysisResult.loading || analysisResult.error) && (
              <div className="p-4 rounded-lg border" style={{ background: 'var(--glass-bg-tertiary)', borderColor: 'var(--glass-border)' }}>
                  ...
                  <div className="flex items-center justify-center py-12">
                    <div className="flex flex-col items-center gap-3">
                      <div 
                        ...
                        style={{ borderColor: 'var(--glass-blue)', borderTopColor: 'transparent' }}
                      />
                ...
                {analysisResult.error && (
                  <div className="p-4 rounded-lg border" style={{ background: 'rgba(239, 68, 68, 0.1)', borderColor: 'var(--glass-red)' }}>
                ...
                {analysisResult.analysis && (
                  <div className="p-4 rounded-lg" style={{ background: 'var(--glass-bg-secondary)' }}>
                    <div className="whitespace-pre-wrap text-sm leading-relaxed" style={{ color: 'var(--glass-text-primary)' }}>
                      {analysisResult.analysis}
                    </div>
                  </div>
                )}
