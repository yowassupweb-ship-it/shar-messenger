...
export async function GET(request: Request) {
  try {
    ...
    if (debug) {
      ...
      // Попробуем прочитать файл напрямую
      let fileContent = '';
      let parseError = '';
      let parsedData: ClustersData | null = null;
      
      try {
        fileContent = fs.readFileSync(clustersPath, 'utf-8');
      ...
      return NextResponse.json({
        ...
        clustersFileExists: fs.existsSync(clustersPath),
        clustersFileSize: fs.existsSync(clustersPath) ? fs.statSync(clustersPath).size : 0,
        files: fs.existsSync(dataDir) ? fs.readdirSync(dataDir) : [],
        fileContentLength: fileContent.length,
        fileContentStart: fileContent.substring(0, 200),
        parseError,
        parsedClustersCount: parsedData?.clusters?.length || 0
      });
    }
    
    console.log('[API/clusters] Reading clusters...');
    const data = readJsonFile<ClustersData>('clusters.json', { clusters: [] });
    console.log('[API/clusters] Got data:', JSON.stringify(data).substring(0, 200));
    console.log('[API/clusters] Clusters count:', data.clusters?.length);
