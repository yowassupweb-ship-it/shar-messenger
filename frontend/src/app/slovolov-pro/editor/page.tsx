...
function EditorPageContent() {
  ...
  // Инициализируем из URL или localStorage сразу
  ...
  const lastSavedFiltersRef = useRef<{filters: string[], apply: boolean, minFrequency: number}>({filters: [], apply: false, minFrequency: 0});
  ...
  // Синхронизация конфигураций на сервер (только на клиенте)
  useEffect(() => {
          ...
          if (configs) {
            ...
            if (config) {
              ...
              if (config.filters && Array.isArray(config.filters) && config.filters.length > 0) {
                ...
                lastSavedFiltersRef.current = {filters: [...config.filters], apply: shouldApply, minFrequency: config.minFrequency || 0};
                setHasFilterChanges(false);
              } else {
                setSelectedFilters([]);
                setApplyFilters(false);
                lastSavedFiltersRef.current = {filters: [], apply: false, minFrequency: 0};
                setHasFilterChanges(false);
              }
  ...
  // Восстановление последнего подкластера при монтировании
  useEffect(() => {
    const urlId = searchParams.get('id');
    const savedId = localStorage.getItem('editor-last-subcluster');
    const targetId = urlId || savedId;
    
    if (targetId && !selectedSubcluster) {
      setSelectedSubcluster(targetId);
      // Обновляем URL если пришли без параметра
      if (!urlId && savedId) {
        window.history.replaceState(null, '', `/editor?id=${encodeURIComponent(savedId)}`);
      }
    }
  }, [searchParams, selectedSubcluster]);
  
  // Раскрытие кластера когда данные загружены и подкластер выбран
  useEffect(() => {
    if (clusters.length === 0 || !selectedSubcluster) return;
    
    const cluster = clusters.find(c => c.types.some(t => t.id === selectedSubcluster));
    if (cluster) {
      setExpandedClusters(prev => ({ ...prev, [cluster.id]: true }));
    } else {
      // Подкластер не найден - сбрасываем
      setSelectedSubcluster(null);
      localStorage.removeItem('editor-last-subcluster');
    }
  }, [clusters, selectedSubcluster]);
  ...
  useEffect(() => {
    ...
    // Проверяем изменились ли фильтры или minFrequency
    const filtersChanged = 
      ...
      minFrequency !== lastSavedFiltersRef.current.minFrequency;
    
    setHasFilterChanges(filtersChanged);
  }, [selectedFilters, applyFilters, selectedSubcluster, minFrequency]);
    ...
    if (typeof window !== 'undefined' && selectedSubcluster) {
      ...
      if (configs) {
        try {
          ...
          if (configIndex !== -1) {
            parsed[configIndex].filters = selectedFilters;
            parsed[configIndex].applyFilters = applyFilters;
            parsed[configIndex].minFrequency = minFrequency;
            localStorage.setItem('subclusterConfigs', JSON.stringify(parsed));
            lastSavedFiltersRef.current = {filters: [...selectedFilters], apply: applyFilters, minFrequency};
            setHasFilterChanges(false);
  ...
  const handleSelectSubcluster = (subclusterId: string) => {
    if (subclusterId === selectedSubcluster) return; // Не перезагружаем если тот же
    setSelectedSubcluster(subclusterId);
    localStorage.setItem('editor-last-subcluster', subclusterId);
    // Обновляем URL без полной навигации
    window.history.replaceState(null, '', `/editor?id=${encodeURIComponent(subclusterId)}`);
  };
      ...
      <div className="h-full flex gap-3 p-3">
        ...
        >
          ...
          <div className="px-2 py-1.5 border-b border-white/10">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const newValue = val === '' ? 0 : Math.max(0, parseInt(val) || 0);
                setMinFrequency(newValue);
              }}
              min="0"
              step="100"
              placeholder="0"
              className="w-full bg-white/5 border border-white/10 rounded px-2 py-1 text-[10px] text-white/80 focus:outline-none focus:border-blue-500/50"
            />
          </div>
          ...
          {applyFilters && selectedFilters.length > 0 && (
            <div 
              className="mx-2 mb-2 rounded-2xl border border-white/20 p-2.5 text-[10px] space-y-0.5"
              style={{
                background: '#1a1a1a',
          ...
          {hasFilterChanges && (
            <div className="px-2 py-2 border-t border-white/10">
              <button 
                onClick={saveFilters}
                className="w-full text-[10px] py-1.5 rounded-lg bg-green-500/20 hover:bg-green-500/30 text-green-400 font-medium transition-colors"
        ...
        <div className="flex-1 flex flex-col gap-3">
          ...
          {/* Content area */}
          <div 
            className="flex-1 rounded-3xl border border-white/20 overflow-hidden flex flex-col relative"
            style={{
              background: '#1a1a1a',
              boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.3)'
            }}
